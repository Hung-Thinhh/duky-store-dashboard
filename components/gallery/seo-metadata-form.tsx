...
export interface GalleryMetadataPayload {
  altText: string
  title: string
  fileName: string
  forMale?: boolean
...
interface GalleryMetadataFormProps {
  ...
  initialForMale?: boolean | null
  onSave: (data: GalleryMetadataPayload) => Promise<void>
  isSaving: boolean
  error?: string | null
  allowDefaultMetadata?: boolean
  defaultMetadataText?: string
  submitLabel?: string
  onDraftChange?: (data: GalleryMetadataPayload) => void
}

export function GalleryMetadataForm({
  originalExtension,
  initialAltText = "",
  initialTitle = "",
  initialFileName = "",
  ...
  const [isManuallyEdited, setIsManuallyEdited] = React.useState(
    Boolean(initialFileName)
  )
  ...
  const resolvePayload = React.useCallback((): GalleryMetadataPayload => {
  ...
  const previewFilename = seoFilename || `gallery-image${originalExtension}`
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          {validationError ? (
            ...
            <p className="text-xs text-stone-500">
              Bỏ trống sẽ tự điền {defaultMetadataText}.
            </p>
          ) : (
            <span />
          )}
      ...
      <div className="space-y-2">
        <Label htmlFor="gallery-for-male">Phân loại giới tính</Label>
        <Select
          value={forMale === undefined ? "all" : String(forMale)}
          onValueChange={(val) => {
            if (val === "all") setForMale(undefined)
            else setForMale(val === "true")
          }}
        >
          <SelectTrigger id="gallery-for-male" className="w-full">
            <SelectValue placeholder="Chọn phân loại giới tính" />
          </SelectTrigger>
          <SelectContent>
