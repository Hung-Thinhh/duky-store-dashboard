...
import type { GalleryImage } from "@/lib/api/schemas/gallery.schema"
...
} from "./seo-metadata-form"

type GalleryUploadDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  onUploaded: (media: GalleryImage | GalleryImage[]) => void | Promise<void>
}
...
type SelectedUpload = {
  ...
  file: File
  previewUrl: string
  metadata: GalleryMetadataPayload
}
...
function looksUglyFileName(fileName: string) {
  ...
  if (
    ...
    return true
  if (
    /^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(base)
  )
    return true
...
function buildDefaultMetadata(file: File): GalleryMetadataPayload {
  const extension = getStoredExtension(file)
  const originalBase = slugify(stripExtension(file.name))
  const needsBrandPrefix =
    looksUglyFileName(file.name) || !originalBase.startsWith("duky-store")
...
export function GalleryUploadDialog({
  ...
  const activeUpload = React.useMemo(
    () =>
      uploads.find((upload) => upload.id === activeUploadId) ??
      uploads[0] ??
      null,
  ...
  const updateActiveMetadata = async (metadata: GalleryMetadataPayload) => {
      ...
      setUploads((current) =>
        current.map((upload) => {
          ...
          if (
            ...
            upload.metadata.forMale === metadata.forMale
          ) {
            return upload
          }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      for (const upload of uploads) {
        uploaded.push(
          await galleryService.uploadGallery(upload.file, upload.metadata)
        )
      }
      ...
      console.error("Failed to upload gallery image with metadata", uploadError)
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-6xl overflow-y-auto rounded-lg p-0">
        <DialogHeader className="border-b border-stone-200 px-6 py-4">
          <DialogTitle>Tải ảnh vào Gallery</DialogTitle>
          <DialogDescription>
            Chọn một hoặc nhiều ảnh cho Gallery. Phân loại giới tính tương ứng
            nếu cần.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="p-8">
            <div className="mx-auto flex min-h-72 max-w-2xl flex-col items-center justify-center rounded-lg border border-dashed border-stone-300 bg-stone-50 p-10 text-center">
              ...
              <h3 className="mt-4 text-lg font-semibold text-stone-900">
                Chọn ảnh để tải lên Gallery
              </h3>
              <p className="mt-2 text-sm text-stone-500">
                Có thể chọn nhiều ảnh cùng lúc. Tên file xấu sẽ được chuẩn hóa
                kèm duky-store.
              </p>
              <Label
                htmlFor="gallery-library-upload"
              ...
              {activeUpload ? (
                <div className="overflow-hidden rounded-lg border border-stone-200 bg-stone-50">
                  ...
                  <img
                    ...
                    alt={
                      activeUpload.metadata.altText || "Xem trước ảnh tải lên"
                    }
                  ...
                  return (
                    ...
                    >
                      <span className="relative flex size-14 shrink-0 overflow-hidden rounded-md bg-stone-100">
                        ...
                        />
                        {isActive ? (
                          <span className="absolute top-1 right-1 rounded-full bg-orange-600 p-0.5 text-white">
                      ...
                      <span className="min-w-0 text-sm">
                        ...
                        <span className="mt-0.5 block text-xs text-stone-500">
                          {getReadableSize(upload.file.size)} -{" "}
            ...
            <div className="space-y-3">
              <div>
                <h3 className="text-base font-semibold text-stone-900">
                  Thông tin SEO & Giới tính
                </h3>
                <p className="mt-1 text-sm text-stone-500">
                  Đang sửa{" "}
                  {uploads.findIndex(
                    (upload) => upload.id === activeUpload?.id
                  ) + 1}
                  /{uploads.length}.
                </p>
              </div>
              ...
              {activeUpload ? (
                <GalleryMetadataForm
                  ...
                  initialForMale={activeUpload.metadata.forMale}
