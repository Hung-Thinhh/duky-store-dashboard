...
const navGroups = [
  {
    label: "Tổng quan",
    items: [
      { title: "Bảng điều khiển", icon: IconDashboard, href: "/", active: true },
    ],
  },
  {
    label: "Đơn hàng",
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Biến thể / Kích thước", icon: IconAdjustments, href: "/variants" },
  ...
  },
  {
    label: "Tiếp thị",
    items: [
      { title: "Công cụ tăng trưởng", icon: IconSpeakerphone, href: "/marketing" },
    ],
  },
  {
    label: "Báo cáo",
    items: [
      { title: "Báo cáo", icon: IconReportAnalytics, href: "/reports" },
    ],
  },
  {
    label: "Hệ thống",
...
export function AppSidebar() {
  ...
  const [passwordFeedback, setPasswordFeedback] = React.useState<{ message: string; tone: "success" | "error" } | null>(null)
  ...
  const handleChangePassword = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (passwordForm.newPassword !== passwordForm.confirmPassword) {
      setPasswordFeedback({ message: "Mật khẩu xác nhận chưa khớp.", tone: "error" })
      return
    }
    ...
    try {
      ...
      setPasswordFeedback({ message: "Đã đổi mật khẩu thành công.", tone: "success" })
    } catch (error) {
      console.error("Failed to change password", error)
      setPasswordFeedback({ message: "Đổi mật khẩu thất bại. Vui lòng kiểm tra mật khẩu hiện tại.", tone: "error" })
    } finally {
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton
              size="lg"
              asChild
              className="hover:bg-transparent active:bg-transparent group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-0! group-data-[collapsible=icon]:justify-center"
            >
              <Link href="/">
              ...
              <SidebarMenu>
                {group.items.map((item) => (
                  <SidebarMenuItem key={item.title}>
                    ...
                    >
                      <Link href={item.href}>
                        <item.icon className="size-[18px] shrink-0 transition-all group-data-[collapsible=icon]:size-[22px]" strokeWidth={1.8} />
          ...
          <SidebarMenuItem>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <SidebarMenuButton
                  size="lg"
                  className="rounded-xl data-[state=open]:bg-secondary group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-0! group-data-[collapsible=icon]:justify-center"
                >
                  ...
                  <div className="flex flex-col gap-0.5 leading-none group-data-[collapsible=icon]:hidden">
                    <span className="text-sm font-medium">{user?.fullName || "Quản trị Duky"}</span>
                    <span className="text-xs text-muted-foreground">
              ...
              >
                <DropdownMenuItem className="rounded-lg" onClick={() => setIsProfileOpen(true)}>
                  <IconUserCircle className="mr-2 size-4" />
                  Hồ sơ
                </DropdownMenuItem>
                <DropdownMenuItem className="rounded-lg" onClick={() => setIsPasswordOpen(true)}>
                  <IconSettings className="mr-2 size-4" />
                  Đổi mật khẩu
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem className="rounded-lg text-destructive cursor-pointer" onClick={() => authService.logout()}>
    ...
    <Dialog open={isProfileOpen} onOpenChange={setIsProfileOpen}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Thông tin admin đang đăng nhập.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4 rounded-xl border bg-card p-4 text-sm">
          <div>
            <span className="mb-1 block text-xs uppercase tracking-wider text-muted-foreground">Họ tên</span>
          ...
          <div>
            <span className="mb-1 block text-xs uppercase tracking-wider text-muted-foreground">Vai trò</span>
            <span>{user?.roles?.join(", ") || "N/A"}</span>
          </div>
        </div>
        <DialogFooter>
          <Button type="button" className="rounded-xl" onClick={() => setIsProfileOpen(false)}>
            Đóng
          </Button>
        </DialogFooter>
      </DialogContent>
      ...
      <DialogContent className="max-w-md">
        <form onSubmit={handleChangePassword} className="space-y-4">
          <DialogHeader>
            ...
            <DialogDescription>Mật khẩu mới cần tối thiểu 8 ký tự.</DialogDescription>
          </DialogHeader>
          <InlineFeedback message={passwordFeedback?.message ?? null} tone={passwordFeedback?.tone} />
          ...
          <DialogFooter>
            <Button type="button" variant="outline" className="rounded-xl" disabled={isSavingPassword} onClick={() => setIsPasswordOpen(false)}>
              Hủy
            </Button>
            <Button type="submit" className="rounded-xl" disabled={isSavingPassword}>
